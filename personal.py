...
class toDoList(Frame):
    def __init__(self):
        ...
        self.master.title("Student Task Management")
    ...
    def setup(self):
        left = tk.Button(self.master, text='<', command=self.go_prev, background='white') #go previous month
        self.wid.append(left)
        left.grid(row=0, column=1)
       
        header = tk.Label(self.master, height=2, text='{}   {}'.format(calendar.month_abbr[self.month], str(self.year)),background='light green')
        self.wid.append(header)
        header.grid(row=0, column=2, columnspan=3,sticky=(N,W,S,E))
       
        right = tk.Button(self.master, text='>', command=self.go_next,background='white') # go next month
        self.wid.append(right)
        right.grid(row=0, column=5)
       
        days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'] # days title overview
        for num, name in enumerate(days):
            t = tk.Label(self.master, text=name,background='black',fg='white')
            self.wid.append(t)
